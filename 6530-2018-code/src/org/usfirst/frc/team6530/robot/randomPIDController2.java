...
public class randomPIDController2 extends SendableBase implements PIDInterface {
	...
	/**
	 * Allocate a PID object with the given constants for P, I, D, with a variable
	 * period, adjusting the constants on the fly to accommodate. This is
	...
	public randomPIDController2(double Kp, double Ki, double Kd, double Kf, PIDSource source, PIDOutput output) {
		...
		instances++;
		// XXX I don't know if these should count or not. I'm not sure how to count them
		// properly if so.
	...
	protected void calculate() {
		...
		// bare open curly -- perfectly legal in Java. Denotes an inner scope for
		// variables.
		{
			double input;
			...
			try {
				...
				timeSinceLastRun = m_periodTimer.get();
			} finally {
				m_thisMutex.unlock();
			}

			if (timeSinceLastRun == 0)
				// just use the constants as they are the first run
				timeSinceLastRun = 1.0;
			...
			if (pidSourceType.equals(PIDSourceType.kRate)) {
				...
				result = P * totalError + D * error / timeSinceLastRun + feedForward;
			} else {
				if (I != 0) {
					totalError = clamp(totalError + error, minimumOutput / I, maximumOutput / I);
				}

				result = P * error + I * totalError * timeSinceLastRun + D * (error - prevError) / timeSinceLastRun
						+ feedForward;
			}
			...
			try {
				...
				// Reset the period timer here, in case pidWrite() takes a nonzero amount of
				// time.
				m_periodTimer.reset();
			}
	...
	public void enable() {
		...
		try {
			if (m_thread == null) {
				...
				m_thread.setName(getName() + " thread");
				m_thread.start();
			}
		} finally {
			m_thisMutex.unlock();
